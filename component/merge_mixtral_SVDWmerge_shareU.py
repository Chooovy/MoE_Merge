...
class Merge_MixtralSparseMoeBlock(nn.Module):
    ...
    def merge_experts(self, module, svd_scale = None):
        ...
        w1 = torch.stack(w1, dim=1).reshape(w1[0].shape[0], -1)
        w2 = torch.stack(w2, dim=1).reshape(w2[0].shape[0], -1)
        w3 = torch.stack(w3, dim=1).reshape(w3[0].shape[0], -1)

        if svd_scale is None:
            shared_u1, concat_v1 = self.svd_delta(w1, rank=self.experts[0].share_low_rank)
            shared_u2, concat_v2 = self.svd_delta(w2, rank=self.experts[0].share_low_rank)
            shared_u3, concat_v3 = self.svd_delta(w3, rank=self.experts[0].share_low_rank)
        else:
            shared_u1, concat_v1 = self.svd_delta(w1, rank=self.experts[0].share_low_rank, svd_scale=scale_w1_mean)
            shared_u2, concat_v2 = self.svd_delta(w2, rank=self.experts[0].share_low_rank, svd_scale=scale_w2_mean)
            shared_u3, concat_v3 = self.svd_delta(w3, rank=self.experts[0].share_low_rank, svd_scale=scale_w3_mean)
        ...
        for j in tqdm(range(self.num_experts), desc="Merging experts", leave=False):
            ...
            self.experts[j].u1.weight = shared_u1
            self.experts[j].v1.weight.data = concat_v1[:, j * self.experts[j].v1.weight.shape[1]:(j + 1) * self.experts[j].v1.weight.shape[1]]
            self.experts[j].u2.weight = shared_u2
            self.experts[j].v2.weight.data = concat_v2[:, j * self.experts[j].v2.weight.shape[1]:(j + 1) * self.experts[j].v2.weight.shape[1]]
            self.experts[j].u3.weight = shared_u3
            self.experts[j].v3.weight.data = concat_v3[:, j * self.experts[j].v3.weight.shape[1]:(j + 1) * self.experts[j].v3.weight.shape[1]]


            # self.experts[j].u1.weight.data, self.experts[j].v1.weight.data = self.svd_delta(module.experts[j].w1.weight, rank=self.experts[j].share_low_rank, svd_scale=scale_w1_mean)
            # self.experts[j].u2.weight.data, self.experts[j].v2.weight.data = self.svd_delta(module.experts[j].w2.weight, rank=self.experts[j].share_low_rank, svd_scale=scale_w2_mean)
            # self.experts[j].u3.weight.data, self.experts[j].v3.weight.data = self.svd_delta(module.experts[j].w3.weight, rank=self.experts[j].share_low_rank, svd_scale=scale_w3_mean)



        for j in range(self.num_experts):
            delta_w1 = module.experts[j].w1.weight - self.experts[j].u1.weight @ self.experts[j].v1.weight
            delta_w2 = module.experts[j].w2.weight - self.experts[j].u2.weight @ self.experts[j].v2.weight
            delta_w3 = module.experts[j].w3.weight - self.experts[j].u3.weight @ self.experts[j].v3.weight
