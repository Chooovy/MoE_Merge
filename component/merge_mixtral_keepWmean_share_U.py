...
class Merge_MixtralSparseMoeBlock(nn.Module):
    ...
    def svd_delta(W, ratio=1, svd_scale=None, rank=None):
        ...
        if svd_scale is None:
            ...
            if W.shape[1] != svd_scale.shape[0]:
                svd_scale_concat = torch.cat([svd_scale] * 8, dim=0)
                W_scale = torch.matmul(W, svd_scale_concat.bfloat16())
                ...
                svd_scale_inv_concat = torch.cat([svd_scale_inv] * 8, dim=1)
                truc_v = torch.matmul(VT[:num_s_after_trunc, :], svd_scale_inv_concat)
                del VT
                truc_sigma = torch.diag(truc_s)
                del truc_s
                sqrtSigma = torch.sqrt(truc_sigma)
    ...
    def merge_experts(self, module, svd_scale = None):
        ...
        for j in tqdm(range(self.num_experts), desc="Merging experts", leave=False):
            ...
            self.experts[j].v1.weight.data = delta_v1[:, j * self.experts[j].v1.weight.shape[1]:(j + 1) * self.experts[j].v1.weight.shape[1]]
            self.experts[j].u2.weight = shared_u2
            self.experts[j].v2.weight.data = delta_v2[:, j * self.experts[j].v2.weight.shape[1]:(j + 1) * self.experts[j].v2.weight.shape[1]]
            self.experts[j].u3.weight = shared_u3
            self.experts[j].v3.weight.data = delta_v3[:, j * self.experts[j].v3.weight.shape[1]:(j + 1) * self.experts[j].v3.weight.shape[1]]
