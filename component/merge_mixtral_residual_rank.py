...
class Merge_MixtralSparseMoeBlock(nn.Module):
    ...
    def merge_experts(self, module, svd_scale = None):
        ...
        for j in tqdm(range(self.num_experts), desc="Merging experts", leave=False):
            ...
            residual_delta_w1 = (module.experts[j].w1.weight - self.expert_mean["w1_mean"]) - self.experts[j].u1.weight @ self.experts[j].v1.weight
            residual_delta_w2 = (module.experts[j].w2.weight - self.expert_mean["w2_mean"]) - self.experts[j].u2.weight @ self.experts[j].v2.weight
            residual_delta_w3 = (module.experts[j].w3.weight - self.expert_mean["w3_mean"]) - self.experts[j].u3.weight @ self.experts[j].v3.weight

            if svd_scale is not None:
                base_name = f"block_sparse_moe.experts.{j}."
                self.experts[j].residual_u1.weight.data, self.experts[j].residual_v1.weight.data = self.svd_delta(residual_delta_w1, ratio=self.delta_ratio, svd_scale=svd_scale[base_name + "w1"])
                self.experts[j].residual_u2.weight.data, self.experts[j].residual_v2.weight.data = self.svd_delta(residual_delta_w2, ratio=self.delta_ratio, svd_scale=svd_scale[base_name + "w2"])
                self.experts[j].residual_u3.weight.data, self.experts[j].residual_v3.weight.data = self.svd_delta(residual_delta_w3, ratio=self.delta_ratio, svd_scale=svd_scale[base_name + "w3"])
            else:
                self.experts[j].residual_u1.weight.data, self.experts[j].residual_v1.weight.data = self.svd_delta(residual_delta_w1, ratio=self.delta_ratio)
                self.experts[j].residual_u2.weight.data, self.experts[j].residual_v2.weight.data = self.svd_delta(residual_delta_w2, ratio=self.delta_ratio)
                self.experts[j].residual_u3.weight.data, self.experts[j].residual_v3.weight.data = self.svd_delta(residual_delta_w3, ratio=self.delta_ratio)
...
class share_expert_with_delta_weight(nn.Module):
    def __init__(self, config: MixtralConfig, share_ratio=1, delta_ratio=1):
        ...
        self.residual_u1 = nn.Linear(self.delta_low_rank, self.intermediate_dim, bias=False, dtype=torch.bfloat16)
        self.residual_v1 = nn.Linear(self.hidden_dim, self.delta_low_rank, bias=False, dtype=torch.bfloat16)

        self.residual_u2 = nn.Linear(self.delta_low_rank, self.hidden_dim, bias=False, dtype=torch.bfloat16)
        self.residual_v2 = nn.Linear(self.intermediate_dim, self.delta_low_rank, bias=False, dtype=torch.bfloat16)

        self.residual_u3 = nn.Linear(self.delta_low_rank, self.intermediate_dim, bias=False, dtype=torch.bfloat16)
        self.residual_v3 = nn.Linear(self.hidden_dim, self.delta_low_rank, bias=False, dtype=torch.bfloat16)

        # v is right, u is left

    def forward(self, hidden_states):
        up = self.w3(hidden_states) + self.u3(self.v3(hidden_states)) + self.residual_u3(self.residual_v3(hidden_states))
        gate = self.w1(hidden_states) + self.u1(self.v1(hidden_states)) + self.residual_u1(self.residual_v1(hidden_states))
        return self.w2(self.act_fn(gate) * up) + self.u2(self.v2(self.act_fn(gate) * up)) + self.residual_u2(self.residual_v2(self.act_fn(gate) * up))
