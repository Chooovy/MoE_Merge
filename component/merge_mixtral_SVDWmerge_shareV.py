...
class Merge_MixtralSparseMoeBlock(nn.Module):
    ...
    def merge_experts(self, module, svd_scale = None):
        ...
        if svd_scale is None:
            concat_u1, shared_v1 = self.svd_delta(w1, rank=self.experts[0].share_low_rank)
            concat_u2, shared_v2 = self.svd_delta(w2, rank=self.experts[0].share_low_rank)
            concat_u3, shared_v3 = self.svd_delta(w3, rank=self.experts[0].share_low_rank)
        else:
            concat_u1, shared_v1 = self.svd_delta(w1, rank=self.experts[0].share_low_rank, svd_scale=scale_w1_mean)
            concat_u2, shared_v2 = self.svd_delta(w2, rank=self.experts[0].share_low_rank, svd_scale=scale_w2_mean)
            concat_u3, shared_v3 = self.svd_delta(w3, rank=self.experts[0].share_low_rank, svd_scale=scale_w3_mean)
        ...
        for j in tqdm(range(self.num_experts), desc="Merging experts", leave=False):
            ...
            self.experts[j].u1.weight.data = concat_u1[j * self.experts[j].u1.weight.shape[0]:(j + 1) * self.experts[j].u1.weight.shape[0], :]
            self.experts[j].v1.weight = shared_v1
            self.experts[j].u2.weight.data = concat_u2[j * self.experts[j].u2.weight.shape[0]:(j + 1) * self.experts[j].u2.weight.shape[0], :]
            self.experts[j].v2.weight = shared_v2
            self.experts[j].u3.weight.data = concat_u3[j * self.experts[j].u3.weight.shape[0]:(j + 1) * self.experts[j].u3.weight.shape[0], :]
