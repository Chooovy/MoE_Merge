    ...
    def merge_experts(self, module, svd_scale = None):
        ...
        for j in range(self.num_experts):
            ...
            if shared_u1 is None:
                shared_u1 = delta_u1[j * self.experts[j].u1.weight.shape[0]:(j + 1) * self.experts[j].u1.weight.shape[0], :] * freq 
            else:
                shared_u1 += delta_u1[j * self.experts[j].u1.weight.shape[0]:(j + 1) * self.experts[j].u1.weight.shape[0], :] * freq
            if shared_u2 is None:
                shared_u2 = delta_u2[j * self.experts[j].u2.weight.shape[0]:(j + 1) * self.experts[j].u2.weight.shape[0], :] * freq
            else:
                shared_u2 += delta_u2[j * self.experts[j].u2.weight.shape[0]:(j + 1) * self.experts[j].u2.weight.shape[0], :] * freq
            if shared_u3 is None:
                shared_u3 = delta_u3[j * self.experts[j].u3.weight.shape[0]:(j + 1) * self.experts[j].u3.weight.shape[0], :] * freq
            else:
                shared_u3 += delta_u3[j * self.experts[j].u3.weight.shape[0]:(j + 1) * self.experts[j].u3.weight.shape[0], :] * freq
