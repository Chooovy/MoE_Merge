...
class Merge_MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, share_ratio, delta_ratio, expert_freq):
        ...
        self.shared_u1 = nn.Linear(self.share_low_rank, self.intermediate_dim, bias=False, dtype=torch.bfloat16)
        self.shared_v1 = nn.Linear(self.hidden_dim, self.share_low_rank, bias=False, dtype=torch.bfloat16)
        self.shared_u2 = nn.Linear(self.share_low_rank, self.hidden_dim, bias=False, dtype=torch.bfloat16)
        self.shared_v2 = nn.Linear(self.intermediate_dim, self.share_low_rank, bias=False, dtype=torch.bfloat16)
        self.shared_u3 = nn.Linear(self.share_low_rank, self.intermediate_dim, bias=False, dtype=torch.bfloat16)
        self.shared_v3 = nn.Linear(self.hidden_dim, self.share_low_rank, bias=False, dtype=torch.bfloat16)
        nn.init.zeros_(self.shared_u1.weight)
        nn.init.zeros_(self.shared_v1.weight)
        nn.init.zeros_(self.shared_u2.weight)
        nn.init.zeros_(self.shared_v2.weight)
        nn.init.zeros_(self.shared_u3.weight)
        nn.init.zeros_(self.shared_v3.weight)



        self.experts = nn.ModuleList([shareUV_share_deltaUV(config, self.shared_u1, self.shared_v1, 
                                                      self.shared_u2, self.shared_v2, self.shared_u3, self.shared_v3, delta_ratio=self.delta_ratio)  
    ...
    def merge_experts(self, module, svd_scale = None, share_V=False, share_U=False):
        ...
        for j in range(self.num_experts):
            ...
            w1.append(module.experts[j].w1.weight)
            w2.append(module.experts[j].w2.weight)
            w3.append(module.experts[j].w3.weight)

        w1 = torch.stack(w1, dim=0).reshape(-1, w1[0].shape[1])
        w2 = torch.stack(w2, dim=0).reshape(-1, w2[0].shape[1])
        w3 = torch.stack(w3, dim=0).reshape(-1, w3[0].shape[1])

        if svd_scale is None:
            concat_u1, shared_v1 = self.svd_delta(w1, rank=self.share_low_rank)
            concat_u2, shared_v2 = self.svd_delta(w2, rank=self.share_low_rank)
            concat_u3, shared_v3 = self.svd_delta(w3, rank=self.share_low_rank)
        else:
            concat_u1, shared_v1 = self.svd_delta(w1, rank=self.share_low_rank, svd_scale=scale_w1_mean)
            concat_u2, shared_v2 = self.svd_delta(w2, rank=self.share_low_rank, svd_scale=scale_w2_mean)
            concat_u3, shared_v3 = self.svd_delta(w3, rank=self.share_low_rank, svd_scale=scale_w3_mean)

        self.shared_v1.weight.data = shared_v1
        self.shared_v2.weight.data = shared_v2
        self.shared_v3.weight.data = shared_v3
        ...
        for j in range(self.num_experts):
            ...
            if shared_u1 is None:
                shared_u1 = concat_u1[j * self.shared_u1.weight.shape[0]:(j + 1) * self.shared_u1.weight.shape[0], :] * freq 
            else:
                shared_u1 += concat_u1[j * self.shared_u1.weight.shape[0]:(j + 1) * self.shared_u1.weight.shape[0], :] * freq
            if shared_u2 is None:
                shared_u2 = concat_u2[j * self.shared_u2.weight.shape[0]:(j + 1) * self.shared_u2.weight.shape[0], :] * freq
            else:
                shared_u2 += concat_u2[j * self.shared_u2.weight.shape[0]:(j + 1) * self.shared_u2.weight.shape[0], :] * freq
            if shared_u3 is None:
                shared_u3 = concat_u3[j * self.shared_u3.weight.shape[0]:(j + 1) * self.shared_u3.weight.shape[0], :] * freq
            else:
                shared_u3 += concat_u3[j * self.shared_u3.weight.shape[0]:(j + 1) * self.shared_u3.weight.shape[0], :] * freq
        ...
        self.shared_u1.weight.data = shared_u1
        self.shared_u2.weight.data = shared_u2
        self.shared_u3.weight.data = shared_u3


        # for j in range(self.num_experts):
        #     delta_w1 = module.experts[j].w1.weight - self.shared_u1.weight @ self.shared_v1.weight
        #     delta_w2 = module.experts[j].w2.weight - self.shared_u2.weight @ self.shared_v2.weight
        #     delta_w3 = module.experts[j].w3.weight - self.shared_u3.weight @ self.shared_v3.weight

        #     self.experts[j].deltaw1.weight.data = delta_w1
        #     self.experts[j].deltaw2.weight.data = delta_w2
        #     self.experts[j].deltaw3.weight.data = delta_w3

        #     if hasattr(module.experts[j].w1, 'W_mask'):
        #         self.experts[j].deltaw1.weight.data[module.experts[j].w1.W_mask] = 0
        #         self.experts[j].deltaw2.weight.data[module.experts[j].w2.W_mask] = 0
        #         self.experts[j].deltaw3.weight.data[module.experts[j].w3.W_mask] = 0
        ...
        if share_V == True and share_U == False:
            ...
            for j in tqdm(range(self.num_experts), desc="Merging experts", leave=False):
                ...
                delta_w1.append(module.experts[j].w1.weight - self.shared_u1.weight @ self.shared_v1.weight)
                delta_w2.append(module.experts[j].w2.weight - self.shared_u2.weight @ self.shared_v2.weight)
                delta_w3.append(module.experts[j].w3.weight - self.shared_u3.weight @ self.shared_v3.weight)
...
class share_svd_expert_with_delta_weight(nn.Module):
    def __init__(self, config: MixtralConfig, share_ratio=1, delta_ratio=1):
        ...
        self.deltaw1 = nn.Linear(self.hidden_dim, self.intermediate_dim, bias=False, dtype=self.dtype)
        self.deltaw2 = nn.Linear(self.intermediate_dim, self.hidden_dim, bias=False, dtype=self.dtype)
        self.deltaw3 = nn.Linear(self.hidden_dim, self.intermediate_dim, bias=False, dtype=self.dtype)
        ...
        self.u1 = nn.Linear(self.share_low_rank, self.intermediate_dim, bias=False, dtype=torch.bfloat16)
        self.v1 = nn.Linear(self.hidden_dim, self.share_low_rank, bias=False, dtype=torch.bfloat16)

        self.u2 = nn.Linear(self.share_low_rank, self.hidden_dim, bias=False, dtype=torch.bfloat16)
        self.v2 = nn.Linear(self.intermediate_dim, self.share_low_rank, bias=False, dtype=torch.bfloat16)

        self.u3 = nn.Linear(self.share_low_rank, self.intermediate_dim, bias=False, dtype=torch.bfloat16)
        self.v3 = nn.Linear(self.hidden_dim, self.share_low_rank, bias=False, dtype=torch.bfloat16)

        # v is right, u is left

    def forward(self, hidden_states):
        up = self.deltaw3(hidden_states) + self.u3(self.v3(hidden_states))
        gate = self.deltaw1(hidden_states) + self.u1(self.v1(hidden_states))
        return self.deltaw2(self.act_fn(gate) * up) + self.u2(self.v2(self.act_fn(gate) * up))
    


class shareUV_deltaW(nn.Module):
    def __init__(self, config: MixtralConfig, shared_u1, shared_v1, shared_u2, shared_v2, shared_u3, shared_v3):
        super().__init__()
        self.intermediate_dim = config.intermediate_size
        self.hidden_dim = config.hidden_size
        self.dtype = torch.bfloat16
        ...
        self.shared_u1 = shared_u1
        self.shared_v1 = shared_v1
        self.shared_u2 = shared_u2
        self.shared_v2 = shared_v2
        self.shared_u3 = shared_u3
        self.shared_v3 = shared_v3
    ...
    def forward(self, hidden_states):
        up = self.deltaw3(hidden_states) + self.shared_u3(self.shared_v3(hidden_states))
        gate = self.deltaw1(hidden_states) + self.shared_u1(self.shared_v1(hidden_states))
        return self.deltaw2(self.act_fn(gate) * up) + self.shared_u2(self.shared_v2(self.act_fn(gate) * up))


class shareUV_share_deltaUV(nn.Module):
    def __init__(self, config: MixtralConfig, shared_u1, shared_v1, shared_u2, shared_v2, shared_u3, shared_v3, delta_ratio=1):
        super().__init__()
        self.intermediate_dim = config.intermediate_size
        self.hidden_dim = config.hidden_size
        self.dtype = torch.bfloat16
    ...
    def forward(self, hidden_states):
        up = self.delta_u3(self.delta_v3(hidden_states)) + self.shared_u3(self.shared_v3(hidden_states))
        gate = self.delta_u1(self.delta_v1(hidden_states)) + self.shared_u1(self.shared_v1(hidden_states))
        return self.delta_u2(self.delta_v2(self.act_fn(gate) * up)) + self.shared_u2(self.shared_v2(self.act_fn(gate) * up))
